/** v1 static option catalogs for category fields with option_source brands/models. */

export const AUTOMOBILE_BRANDS = [
  "Audi",
  "BMW",
  "BYD",
  "Changan",
...
export const ELECTRONICS_BRANDS_BY_SUBCATEGORY = {
...
export function normalizeOptionKey(value) {
  return String(value ?? "").trim().normalize("NFKC").toLocaleLowerCase("az");
}
...
function resolveRecordOptions(catalog, key) {
  const normalizedKey = normalizeOptionKey(key);
  if (!normalizedKey) return [];
  const catalogKey = Object.keys(catalog).find((item) => normalizeOptionKey(item) === normalizedKey);
  return catalogKey ? catalog[catalogKey] ?? [] : [];
}

export function resolveBrandOptions(categoryKey, subcategorySlug) {
...
export function resolveModelOptions(categoryKey, brand) {
  if (categoryKey === "automobile" || categoryKey === "auto_parts") return dedupeOptions(resolveRecordOptions(AUTOMOBILE_MODELS, brand));
  if (categoryKey === "phone") return dedupeOptions(resolveRecordOptions(PHONE_MODELS, brand));
  return [];
}
