-- Manual rollback for 20260720124000_store_claim_rpc_authorization_alignment.sql.
-- Restores the previous submit_store_claim_request RPC body only.
-- The same-store pending unique index is baseline protection and is intentionally not dropped.
...
begin
  ...
  if p_claim_code is null or btrim(p_claim_code) = '' then
    raise exception 'Claim kodunu daxil edin.';
  end if;
