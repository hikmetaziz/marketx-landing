-- Unique MarktX user codes (MX-000001, MX-000002, ...)
...
-- Backfill existing profiles in stable order without changing other columns.
with numbered as (
  select
    id,
...
select setval(
  ...
  coalesce((select max(user_number) from public.profiles), 0),
  true
);
