...
-- Phase 2 conversation types need nullable compatibility fields. Legacy rows
-- keep buyer_id/seller_id/listing_id populated by policy and verification.
alter table public.conversations
...
alter table public.messages
  drop constraint if exists messages_metadata_safe_check,
  add constraint messages_metadata_safe_check check (
    jsonb_typeof(metadata) = 'object'
    and octet_length(metadata::text) <= 4096
    and not (metadata ? 'admin_id')
    and not (metadata ? 'store_owner_id')
    and not (metadata ? 'sender_id')
  ) not valid;
...
begin
  ...
  if v.conversation_type = 'customer_store' then
    ...
    -- Support/admin access to private customer-store chats must go through an
    -- audited SECURITY DEFINER RPC or a future restricted view, not direct RLS.
    return false;
  end if;
...
begin
  ...
  where id = p_conversation_id
    ...
    and (
      ...
      or p_access_reason in ('escalated', 'moderation', 'legal', 'security', 'support_assignment')
    );
...
-- Keep deployed legacy clients working in Phase 2A. Existing direct legacy
-- policies stay in place; new store/support access is additive.
