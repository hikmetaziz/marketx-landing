-- MarktX store claim RPC authorization alignment.
-- Keeps the product rule at one pending claim per same user + same store.
-- Does not enforce a global one-pending-claim-per-user rule.
...
begin
  if exists (
    ...
    raise exception 'store_claim_pending_same_store_duplicates_exist';
  end if;
end $$;
...
begin
  ...
  where s.store_code = btrim(coalesce(p_store_code, ''));
  ...
  if v_code.claim_code_hash <> extensions.crypt(btrim(p_claim_code), v_code.claim_code_hash) then
    raise exception '%', v_invalid_message;
  end if;
  ...
  perform public.store_audit(
    ...
    jsonb_build_object('request_id', v_request_id, 'with_code', true)
  );
