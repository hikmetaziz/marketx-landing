-- MarktX store claim-code activation: allow existing members to claim another store.
-- A user's membership in Store A must not block a valid admin-issued code for Store B.
...
comment on function public.submit_store_claim_request(text, text, text, text, text) is
  'User-facing store-code + claim-code activation. A valid admin-issued code directly assigns owner access; existing membership in another store does not block activation.';
