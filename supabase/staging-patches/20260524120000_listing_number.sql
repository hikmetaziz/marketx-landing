-- Staging-safe copy of mobile migration 20260524120000_listing_number.sql.
-- Difference: setval handles an empty listings table without setting sequence to 0.
...
select setval(
  ...
  greatest(coalesce((select max(listing_number) from public.listings), 1), 1),
  coalesce((select max(listing_number) from public.listings), 0) > 0
);
